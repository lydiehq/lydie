...
export const bloggerIntegration: Integration & OAuthIntegration = {
  ...
  async push(options: PushOptions): Promise<SyncResult> {
    ...
    if (!blogId) {
      return createErrorResult(
        document.id,
        "Blog ID is required. Please select a blog resource."
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Blogger API error: ${response.status} ${response.statusText} - ${errorText}`
        );
      }
      ...
      return createErrorResult(
        document.id,
        error.message || "Failed to push to Blogger"
      );
    }
  },
  ...
  async delete(options: DeleteOptions): Promise<SyncResult> {
    ...
    try {
      ...
      return createErrorResult(
        documentId,
        error.message || "Failed to delete from Blogger"
      );
    }
  },
    ...
    try {
      ...
      if (blogId) {
        ...
        if (blogsRes.ok) {
          ...
          if (blogsData.items) {
            blogsToProcess.push(
              ...blogsData.items.map((b) => ({ id: b.id, name: b.name }))
            );
          }
        }
      }
