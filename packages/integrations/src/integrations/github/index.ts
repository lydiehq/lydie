...
export const githubIntegration: GitHubIntegrationExtended = {
  ...
  async push(options: PushOptions): Promise<SyncResult> {
    ...
    try {
      ...
      if (!putResponse.ok) {
        ...
        throw new Error(`Failed to push file: ${putResponse.statusText} - ${JSON.stringify(errorData)}`)
      }
  ...
  async delete(options: DeleteOptions): Promise<SyncResult> {
    ...
    try {
      ...
      if (!deleteResponse.ok) {
        ...
        throw new Error(`Failed to delete file: ${deleteResponse.statusText} - ${JSON.stringify(errorData)}`)
      }
  ...
  async fetchResources(connection: IntegrationConnection): Promise<ExternalResource[]> {
    ...
    if (!installationResponse.ok) {
      ...
      throw new Error(
        `Failed to fetch installation: ${installationResponse.statusText} - ${JSON.stringify(errorData)}`,
      )
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to fetch repositories: ${response.statusText} - ${JSON.stringify(errorData)}`)
    }
  ...
  async onDisconnect(connection: IntegrationConnection): Promise<void> {
    ...
    try {
      ...
      if (!deleteResponse.ok) {
        ...
        const errorData = await deleteResponse.json().catch(() => ({}))
        throw new Error(
          `Failed to delete installation: ${deleteResponse.statusText} - ${JSON.stringify(errorData)}`,
        )
      }
