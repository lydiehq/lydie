...
export class ShopifyIntegration
    ...
    async validateConnection(connection: IntegrationConnection): Promise<{
        ...
        try {
            const response = await fetch(`https://${config.shop}/admin/api/2024-01/shop.json`, {
                headers: {
    ...
    async push(options: PushOptions): Promise<SyncResult> {
        ...
        if (!config.shop || !config.accessToken) {
            return this.createErrorResult(document.id, "Missing Shopify configuration");
        }
        ...
        try {
            ...
            if (resourceType === "pages") {
                ...
                const searchRes = await fetch(`https://${config.shop}/admin/api/2024-01/pages.json?handle=${slug}`, { headers });
                if (searchRes.ok) {
                    const searchData = await searchRes.json() as { pages: Array<{ id: number; handle: string }> };
                    // Exact match check
                ...
                if (existingId) {
                    ...
                    method = "PUT";
                    payload = { page: { id: existingId, title, body_html: htmlContent, handle: slug } };
                } else {
                ...
                if (!blogId) {
                    return this.createErrorResult(document.id, "Blog ID is required for pushing blog articles.");
                }

                // Verify blog exists first (optional but good for robustness)
                const blogCheck = await fetch(`https://${config.shop}/admin/api/2024-01/blogs/${blogId}.json`, { headers });
                if (!blogCheck.ok) {
                    return this.createErrorResult(document.id, "Target Blog not found on Shopify.");
                }

                // 1. Check if article exists in this blog by handle
                const searchRes = await fetch(`https://${config.shop}/admin/api/2024-01/blogs/${blogId}/articles.json?handle=${slug}`, { headers });
                if (searchRes.ok) {
                    const searchData = await searchRes.json() as { articles: Array<{ id: number; handle: string }> };
                ...
                if (existingId) {
                    ...
                    method = "PUT";
                    payload = { article: { id: existingId, title, body_html: htmlContent, handle: slug } };
                } else {
                    endpoint = `https://${config.shop}/admin/api/2024-01/blogs/${blogId}/articles.json`;
                    method = "POST";
                    payload = { article: { title, body_html: htmlContent, handle: slug } };
                }
            } else {
                return this.createErrorResult(document.id, `Unknown resource type: ${resourceType}`);
            }
            ...
            return {
                ...
                metadata: {
                    shop: config.shop,
                    handle: slug,
                    url: `https://${config.shop}/${resourceType === "pages" ? "pages" : "blogs/" + (data.article?.blog_id || resourceId)}/${slug}`
                }
            };

        } catch (error: any) {
            console.error("Shopify Push Error:", error);
            return this.createErrorResult(document.id, error.message || "Failed to push to Shopify");
        }
    }
    ...
    async pull(options: PullOptions): Promise<SyncResult[]> {
        ...
        try {
            // 1. Fetch Pages
            const pagesRes = await fetch(`https://${config.shop}/admin/api/2024-01/pages.json?limit=250`, { headers });
            if (pagesRes.ok) {
                const pagesData = await pagesRes.json() as { pages: Array<{ id: number; title: string, body_html: string, handle: string }> };
            ...
            if (blogsRes.ok) {
                const blogsData = await blogsRes.json() as { blogs: Array<{ id: number; title: string }> };

                for (const blog of blogsData.blogs) {
                    const articlesRes = await fetch(`https://${config.shop}/admin/api/2024-01/blogs/${blog.id}/articles.json?limit=250`, { headers });
                    if (articlesRes.ok) {
                        const articlesData = await articlesRes.json() as { articles: Array<{ id: number; title: string; body_html: string; handle: string }> };
