...
import { OAuthState, decodeOAuthState, encodeOAuthState } from "@lydie/core/integrations/oauth";
import { pullFromIntegrationLink } from "@lydie/core/integrations/pull";
import { Integration, OAuthIntegration } from "@lydie/core/integrations/types";
...
export const IntegrationsRoute = new Hono<{
  ...
  .get("/:type/oauth/callback", async (c) => {
    ...
    if (error) {
      ...
      return c.redirect(
        `${frontendUrl}/settings/integrations?error=${encodeURIComponent(errorDescription)}`,
      );
    }
