...
import {
  ...
  test,
  createUser2Context,
} from "./fixtures/auth-multi-user.fixture";
  ...
  singleUserTest(
    ...
    async ({ page: tab1, context, organization }) => {
      ...
      try {
        ...
        await expect(tab2ContentEditor).toContainText(
          "Initial content - Added from Tab 1",
          { timeout: 5000 }
        );
...
test.describe("real-time collaboration - different users", () => {
  test("two users can edit the same document simultaneously", async ({
    ...
    try {
      ...
      await expect(user2ContentEditor).toContainText(
        "Initial content - Added by User 1",
        { timeout: 5000 }
      );
  ...
  test("title changes sync between users", async ({
    ...
    try {
      ...
      await expect(
        user1Sidebar.getByRole("row", { name: "Updated Title" })
      ).toBeVisible();
      await expect(
        user2Sidebar.getByRole("row", { name: "Updated Title" })
      ).toBeVisible();
    } finally {
      await user2Context.close();
    }
  ...
  test("title syncs only on blur (not real-time like content)", async ({
    ...
    try {
      ...
      await expect(
        user2Sidebar.getByRole("row", { name: "Initial Title - Typing" })
