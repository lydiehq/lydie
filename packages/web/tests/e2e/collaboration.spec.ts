...
import { expect, test, createUser2Context } from "./fixtures/auth-multi-user.fixture"
...
test.describe("real-time collaboration - different users", () => {
  test("two users can edit the same document simultaneously", async ({
    ...
    try {
      ...
      await expect(user1ContentEditor).toContainText("Initial content - Added by User 1 - Added by User 2", {
        timeout: 5000,
      })
  ...
  test("title changes sync between users", async ({ page: user1Page, browser, workerData, organization }) => {
    const baseURL = user1Page.url() || "http://localhost:3000"
    const user2Context = await createUser2Context(browser, workerData, baseURL)
    const user2Page = await user2Context.newPage()
  ...
  test("title syncs only on blur (not real-time like content)", async ({
    ...
    try {
      ...
      await expect(user2ContentEditor).toContainText("Initial content - Real-time", { timeout: 5000 })
...
async function createDocument(page: Page, options: { title: string; content: string }): Promise<string> {
