...
import { db, extensionConnectionsTable, extensionLinksTable } from "@lydie/database";
...
test.describe("Extensions", () => {
  test.describe("GitHub Connection Flow", () => {
    test("should open connection dialog and show extension options", async ({
      ...
      await page.goto(`/w/${organization.id}/settings/extensions`);
      ...
      await expect(page.getByText("Sync documents as Markdown files to a GitHub repository")).toBeVisible();
    ...
    test("should initiate GitHub OAuth flow when connecting", async ({
      ...
      await page.route("**/internal/extensions/github/oauth/authorize", async (route: Route) => {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
          body: JSON.stringify({
            authUrl: "https://github.com/login/oauth/authorize?client_id=test&redirect_uri=http://localhost:3000/oauth/callback&scope=repo&state=test-state",
          }),
        });
      });
      ...
      await page.route("https://github.com/login/oauth/authorize*", async (route: Route) => {
    ...
    test("should handle successful OAuth callback", async ({
      ...
      await page.goto(
        `/w/${organization.id}/settings/extensions?success=true&connectionId=test-connection-123`
      );
      
      // Should show success toast
      await expect(page.getByText("Extension connected successfully!")).toBeVisible();
    ...
    test("should handle OAuth callback error", async ({
      ...
      await page.goto(
        `/w/${organization.id}/settings/extensions?error=access_denied`
      );
      
      // Should show error toast
      await expect(page.getByText(/Failed to connect: access_denied/i)).toBeVisible();
  ...
  test.describe("Extension Links", () => {
    test("should show 'no extensions connected' state initially", async ({
      page,
      organization,
    }) => {
      await page.goto(`/w/${organization.id}/settings/extensions`);
      
      await expect(page.getByText("No extensions connected yet")).toBeVisible();
      await expect(page.getByText("Connect your first extension to start syncing documents")).toBeVisible();
    });
    ...
    test("should allow creating a link after connection exists", async ({
      ...
      await page.route(
        `**/internal/extensions/${connection.id}/links`,
        async (route: Route) => {
      ...
      await page.route(
        "**/internal/extensions/links/test-link-123/sync",
        async (route: Route) => {
          if (route.request().method() === "POST") {
            await route.fulfill({
              status: 200,
      ...
      await expect(page.getByText("You have a connection set up")).toBeVisible();
      ...
      await expect(page.getByText("Loading repositories...")).not.toBeVisible({ timeout: 5000 });
      
      // Fill in link details
      await page.getByPlaceholder("e.g. Web Docs, API Reference").fill("Web Documentation");
      ...
      await expect(page.getByText(/Imported 5 document\(s\)/i)).toBeVisible({ timeout: 10000 });
      
      // Cleanup
      await db
        .delete(extensionConnectionsTable)
    ...
    test("should show and manage existing links", async ({
      ...
      // Cleanup
      await db.delete(extensionLinksTable).where(eq(extensionLinksTable.id, link.id));
      await db
        .delete(extensionConnectionsTable)
        .where(eq(extensionConnectionsTable.id, connection.id));
    });
    ...
    test("should allow syncing a link manually", async ({
      ...
      // Mock the sync endpoint
      await page.route(
        `**/internal/extensions/links/${link.id}/sync`,
        async (route: Route) => {
          if (route.request().method() === "POST") {
            await route.fulfill({
              status: 200,
      ...
      // Should show success toast
      await expect(page.getByText(/Synced 3 document\(s\) from "Test Docs"/i)).toBeVisible({ timeout: 5000 });
    ...
    test("should allow deleting a link", async ({
      ...
      await page.route(
        `**/internal/extensions/links/${link.id}`,
        async (route: Route) => {
    ...
    test("should disable 'Sync Now' when connection or link is disabled", async ({
      ...
      await expect(page.getByRole("menuitem", { name: "Sync Now" })).not.toBeVisible();
  ...
  test.describe("Connection Management", () => {
    ...
    test("should allow deleting a connection", async ({
      ...
      await expect(page.getByText(/All sync metadata for this connection will be permanently deleted/i)).toBeVisible();
      
      // Confirm
      await page.getByRole("button", { name: /Confirm/i }).click();
      
      // Should show success
      await expect(page.getByText("Connection deleted successfully")).toBeVisible();
