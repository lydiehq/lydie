...
test.describe("command menu", () => {
  test("can open command menu with keyboard shortcut", async ({ page, organization }) => {
  ...
  test("can open command menu with sidebar button", async ({ page, organization }) => {
    await page.goto(`/w/${organization.slug}`)
    await page.waitForLoadState("networkidle")
  ...
  test("can navigate to home via command menu", async ({ page, organization }) => {
  ...
  test("can navigate to assistant via command menu", async ({ page, organization }) => {
    await page.goto(`/w/${organization.slug}`)
    await page.waitForLoadState("networkidle")
  ...
  test("should search documents", async ({ page, organization, user }) => {
    ...
    try {
      ...
      await page.getByRole("option", { name: "Search documents…" }).click()
      ...
      await expect(page.getByRole("option", { name: documentTitle1 })).toBeVisible({ timeout: 5000 })
