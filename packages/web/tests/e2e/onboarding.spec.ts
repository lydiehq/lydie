...
import { getOnboardingStatus, updateOnboardingStatus } from "./utils/onboarding";
...
test.describe("onboarding checklist tracking", () => {
  ...
  test("should not allow manually checking checklist items (read-only)", async ({
    page,
    organization,
  }) => {
    await page.goto(`/w/${organization.slug}/assistant`);
...
test.describe("demo guide creation", () => {
  test("should create interactive guide and auto-check checklist items", async ({
    ...
    await expect(
      page.getByRole("option", {
        name: /Create Interactive Guide.*Recommended/i,
      }),
    ).toBeVisible();
  ...
  test("should not show demo guide option after it's been created", async ({
    page,
    organization,
  }) => {
