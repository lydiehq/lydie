...
>({
  workerData: [
    async ({}, use, workerInfo) => {
      ...
      const { user: user1, organization, cleanup: cleanup1 } = await createTestUser({
        prefix: "collab-user1",
        userId: user1Id,
      ...
      try {
        ...
        await db
          .delete(membersTable)
          .where(eq(membersTable.userId, user2Id));
  ...
  workerStorageState: [
    async ({ workerData }, use, workerInfo) => {
      ...
      try {
        ...
        console.error(
          `Failed to cleanup storage state file for worker ${id}:`,
          error
        );
      }
    },
