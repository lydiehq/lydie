...
import { Select as AriaSelect, SelectValue, ListBox } from "react-aria-components"
      ...
      <Popover className="min-w-[250px] max-h-[400px] flex flex-col p-0" placement="bottom start">
        <ListBox items={agents} className="outline-none overflow-auto" selectionMode="single">
          {(agent: any) => {
            ...
            return (
              <SelectItem id={agent.id} textValue={agent.name} className={isSelected ? "bg-blue-50" : ""}>
                <div className="flex flex-col items-start gap-1 w-full max-w-sm">
                  ...
                  {agent.description && <span className="text-xs text-gray-500">{agent.description}</span>}
                </div>
              </SelectItem>
            )
          }}
