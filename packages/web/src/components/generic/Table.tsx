...
export function Table({ className: containerClassName, ...props }: TableProps & { className?: string }) {
  return (
    <ResizableTableContainer
      className={twMerge(
        "max-h-[280px] w-[550px] overflow-auto scroll-pt-[2.281rem] relative",
        containerClassName,
      )}
    >
...
const resizerStyles = cva({
  base: "w-px px-[8px] translate-x-[8px] box-content py-1 h-4 bg-clip-content bg-gray-200 cursor-col-resize rounded-xs resizing:bg-blue-600 forced-colors:resizing:bg-[Highlight] resizing:w-[2px] resizing:pl-[7px] -outline-offset-2",
})
...
export function TableHeader<T extends object>(props: TableHeaderProps<T>) {
  ...
  return (
    <AriaTableHeader {...props} className={composeTailwindRenderProps(props.className, "sticky top-0 z-10")}>
