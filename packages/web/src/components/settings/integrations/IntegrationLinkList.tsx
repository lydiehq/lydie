      ...
      {links.length > 0 ? (
        ...
        <div className="w-full rounded-lg ring ring-black/8 bg-white p-8">
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500 mt-1">Add a link to start syncing documents.</p>
          </div>
        </div>
      )}
    </div>
    ...
    <ul className="p-2.5 hover:bg-black/1 first:rounded-t-lg last:rounded-b-lg transition-colors duration-75">
      ...
      <div className="flex items-center gap-6">
        <div className="text-sm text-gray-600 min-w-[120px]">
          Last synced:
          {link.last_synced_at ? formatDistanceToNow(link.last_synced_at) : "Never"}
        </div>
        <MenuTrigger>
          <RACButton>
    ...
    <Modal isOpen={isOpen} onOpenChange={onOpenChange} isDismissable size="sm">
      <Dialog role="alertdialog">
        <div className="p-4 flex flex-col gap-y-3">
          <RACHeading slot="title" className="text-lg font-medium text-gray-900">
            Confirm
          </RACHeading>
