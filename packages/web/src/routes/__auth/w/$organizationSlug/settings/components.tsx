...
function RouteComponent() {
  ...
  const form = useAppForm({
    ...
    onSubmit: async (values) => {
      const properties = Object.fromEntries(values.properties.map((prop) => [prop.key, { type: prop.type }]))
    ...
    <div className="flex flex-col gap-y-6">
      ...
      <div className="">
        <div className="flex flex-col gap-y-2">
          ...
          <p className="text-sm/relaxed text-gray-700">Create reusable components for your documents.</p>
        </div>
        <Form
          onSubmit={(e) => {
            e.preventDefault()
        ...
        >
          <form.AppField
            name="name"
            children={(field) => <field.TextField label="Block Name" placeholder="Enter block name" />}
          />
