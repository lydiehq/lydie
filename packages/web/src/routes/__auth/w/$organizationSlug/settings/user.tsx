...
import { rootFontSizeAtom, FONT_SIZE_MAP, type FontSizeOption } from "@/stores/font-size"
...
function RouteComponent() {
  ...
  const handleTogglePersistDocumentTreeExpansion = async (isSelected: boolean) => {
    try {
      ...
      toast.success(
        isSelected ? "Document tree expansion will be saved" : "Document tree expansion will not be saved",
      )
    } catch (error) {
        ...
        <Card className="p-4 flex flex-col gap-y-4">
          <div className="flex items-center justify-between">
            <div className="flex flex-col gap-y-1">
              <Label id="persist-document-tree-expansion-label" className="text-sm font-medium text-gray-900">
                Persist Document Tree Expansion
              </Label>
              <p className="text-xs text-gray-500">
                Save the expanded state of documents in the document tree to local storage. When enabled, your
                expanded folders will remain open after refreshing the page.
              </p>
            </div>
            <Switch
          ...
          <div className="flex flex-col gap-y-4">
            <div className="flex flex-col gap-y-1">
              ...
              <p className="text-xs text-gray-500">
                Adjust the base font size for the entire application. This affects all text sizes since they
                use REM units. Changes are saved locally and persist across sessions.
              </p>
            </div>
            <div className="w-48">
              <Select
