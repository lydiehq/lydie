...
export function generateParagraphChunks(
  ...
  function getHeaderBreadcrumb(): {
    ...
    const breadcrumb = headingStack.map((h) => `${"#".repeat(h.level)} ${h.text}`).join(" > ")
  ...
  function flushPendingContent() {
    ...
    if (plainTextLength >= minChunkSize) {
      ...
      const contentWithContext = breadcrumb ? `${breadcrumb}\n\n${content}` : content
      ...
      try {
        ...
        if (plainText.length > 0) {
          if (plainText.length >= minChunkSize * 2) {
            ...
            const accumulatedLength = stripHtmlTags(pendingContent.join("\n")).length
...
export function generateHeadingAwareChunks(
  ...
  for (const item of items) {
    if (item.type === "heading") {
      ...
      const overlapLength =
        previousChunkOverlapHtml.length > 0 ? stripHtmlTags(previousChunkOverlapHtml.join("\n")).length : 0
      const currentPlainLength = stripHtmlTags(currentChunkHtml.join("\n")).length
      const itemPlainLength = item.plainText.length
      const combinedLength = overlapLength + currentPlainLength + itemPlainLength
...
export function generateSimpleChunks(text: string, maxChunkSize: number = 300): Chunk[] {
