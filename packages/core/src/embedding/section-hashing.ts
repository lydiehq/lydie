...
export function extractSections(jsonContent: any): Section[] {
  ...
  const hasSectionHeadings = contentArray.some(
    (node: any) => node.type === "heading" && (node.attrs?.level === 1 || node.attrs?.level === 2),
  )

  if (!hasSectionHeadings) {
    const plaintext = contentArray.map((n: any) => serializeToPlainText(n)).join("\n")
  ...
  contentArray.forEach((node: any, index: number) => {
    ...
    const isSectionHeading = isHeading && (node.attrs?.level === 1 || node.attrs?.level === 2)

    if (isSectionHeading) {
      if (currentSection) {
        const plaintext = currentSection.content.map((n: any) => serializeToPlainText(n)).join("\n")
  ...
  if (currentSection !== null) {
    ...
    if (section.content.length > 0) {
      const plaintext = section.content.map((n: any) => serializeToPlainText(n)).join("\n")
...
export function findChangedSections(
  ...
  const deletedSections = Object.keys(oldHashes).filter((key) => !newKeys.includes(key))
