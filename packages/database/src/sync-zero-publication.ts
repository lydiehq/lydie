...
async function main() {
  ...
  try {
    ...
    if (!pubExists) {
      ...
      console.log(`✅ Successfully created publication '${PUBLICATION_NAME}' for ${allTables.length} tables.`)
    } else {
      ...
      if (tablesToAdd.length === 0 && tablesToRemove.length === 0) {
        ...
        if (tablesToRemove.length > 0) {
          ...
          await db.execute(sql.raw(`ALTER PUBLICATION "${PUBLICATION_NAME}" DROP TABLE ${removeList}`))
        } else {
