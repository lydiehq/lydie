...
async function main() {
  ...
  try {
    ...
    if (!pubExists) {
      console.log(
        `Publication '${PUBLICATION_NAME}' does not exist. Creating it...`
      );
      // CREATE PUBLICATION "name" FOR TABLE "t1", "t2"...
      // PlanetScale doesn't support FOR ALL TABLES, so we list them explicitly
      await db.execute(
        sql.raw(
          `CREATE PUBLICATION "${PUBLICATION_NAME}" FOR TABLE ${tableList}`
        )
      );
      console.log(
      ...
      console.log(
        `Publication '${PUBLICATION_NAME}' exists. Checking for table changes...`
      );
      ...
      const currentPubTables = new Set(
        pubTablesResult.map((row: any) => row.tablename as string)
      );

      const tablesToAdd = allTables.filter((t) => !currentPubTables.has(t));
      const tablesToRemove = [...currentPubTables].filter(
        (t) => !allTables.includes(t)
      );
