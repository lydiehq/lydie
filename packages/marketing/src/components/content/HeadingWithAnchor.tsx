...
export function HeadingWithAnchor({ level, headingId, children }: HeadingWithAnchorProps) {
  const safeLevel = level >= 1 && level <= 6 ? level : 1;
  const headingTag = `h${safeLevel}`;

  return createElement(
    headingTag,
    { id: headingId, "data-copy-heading": true, className: "group/heading relative scroll-mt-28" },
    <span>{children}</span>,
    <a
      href={`#${headingId}`}
      data-copy-heading-link
      aria-label="Copy link to this heading"
      className="ml-2 inline-flex size-6 align-middle items-center justify-center rounded text-gray-400 opacity-0 transition hover:bg-gray-100 hover:text-blue-600 group-hover/heading:opacity-100 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-blue-500/40"
    >
      <svg
        width="16"
        height="16"
